...
export async function* streamState(
  ...
  async function* inner() {
    ...
    if (!LANGGRAPH_VERSION) {
      ...
      LANGGRAPH_VERSION = version.find(
        (v) => v.name === "@langchain/langgraph",
      );
    }
      ...
      if (event.event === "on_chain_stream" && event.run_id === run.run_id) {
        ...
        if (mode === "debug") {
          ...
          if (debugChunk.type === "checkpoint") {
            ...
            const debugResult = preprocessDebugCheckpointTask(
              debugChunk.payload,
            );
  ...
  if (run.kwargs?.config?.configurable?.langsmith_project) {
    const projectName = run.kwargs?.config?.configurable[
      "langsmith_project"
    ] as string;
    const updates: Record<string, string> | null = run.kwargs?.config
      ?.configurable?.langsmith_example_id
      ? {
          reference_example_id: run.kwargs?.config?.configurable
            ?.langsmith_example_id as string,
        }
      : null;
    const tracingConfig = {
      replicas: [
        [projectName, updates],
        ["default", null],
      ] as Array<[string, Record<string, any> | null]>,
    };
    const traceableInner = traceable(inner, tracingConfig);

    for await (const item of traceableInner()) {
      yield item;
    }
  } else {
